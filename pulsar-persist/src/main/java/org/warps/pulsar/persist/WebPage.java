...
import org.warps.pulsar.persist.gora.generated.GParseStatus;
import org.warps.pulsar.persist.gora.generated.GProtocolStatus;
import org.warps.pulsar.persist.gora.generated.GWebPage;
import org.warps.pulsar.persist.metadata.*;
...
import static org.warps.pulsar.persist.metadata.Name.*;
...
public class WebPage {
     ...
     * Set crawl status
     *
     * @see org.warps.pulsar.persist.CrawlStatus
     */
    ...
    public Duration getLastTimeout() {
        String s = getMetadata().get(RESPONSE_TIME);
    ...
    public String getFetchTimeHistory(String defaultValue) {
        String s = getMetadata().get(FETCH_TIME_HISTORY);
    ...
    public void putFetchTimeHistory(Instant fetchTime) {
        String fetchTimeHistory = getMetadata().get(FETCH_TIME_HISTORY);
        fetchTimeHistory = DateTimeUtil.constructTimeHistory(fetchTimeHistory, fetchTime, 10);
        getMetadata().set(FETCH_TIME_HISTORY, fetchTimeHistory);
    }
    ...
    public void setEncoding(String encoding) {
        ...
        getMetadata().set(CHAR_ENCODING_FOR_CONVERSION, encoding);
    }
    ...
    public String getEncodingClues() {
        return getMetadata().getOrDefault(ENCODING_CLUES, "");
    }

    public void setEncodingClues(String clues) {
        getMetadata().set(ENCODING_CLUES, clues);
    }
...
//        }
    ...
    public String[] getInlinkAnchors() {
        return StringUtils.split(getMetadata().getOrDefault(ANCHORS, ""), "\n");
    }

    public void setInlinkAnchors(Collection<CharSequence> anchors) {
        getMetadata().set(ANCHORS, StringUtils.join(anchors, "\n"));
    }
    ...
    public String getIndexTimeHistory(String defaultValue) {
        String s = getMetadata().get(INDEX_TIME_HISTORY);
        return s == null ? defaultValue : s;
    }

    public void putIndexTimeHistory(Instant indexTime) {
        String indexTimeHistory = getMetadata().get(INDEX_TIME_HISTORY);
        indexTimeHistory = DateTimeUtil.constructTimeHistory(indexTimeHistory, indexTime, 10);
        getMetadata().set(INDEX_TIME_HISTORY, indexTimeHistory);
    }
