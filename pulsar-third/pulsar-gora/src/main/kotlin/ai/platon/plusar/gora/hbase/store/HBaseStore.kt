...
class HBaseStore<K, T : PersistentBase> : DataStoreBase<K, T>(), Configurable {
    ...
    override fun get(key: K, fields: Array<String>): T? {
        ...
        try {
            ...
            val get = Get(ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(key))
    ...
    override fun put(key: K, persistent: T) {
        try {
            ...
            val keyRaw = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(key)
        ...
        when (type) {
            Schema.Type.UNION -> if (isNullable(schema) && o == null) {
                ...
                if (index > 1) {  //if more than 2 type in union, serialize directly for now
                    val serializedBytes = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(o, schema)
            ...
            Schema.Type.MAP -> {
                ...
                for ((key, value) in set) {
                    val qual = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(key)
    ...
    override fun delete(key: K): Boolean {
        try {
            table.delete(Delete(ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(key)))
    ...
    override fun deleteByQuery(query: Query<K, T>): Long {
        try {
            ...
            while (result.next()) {
                val delete = Delete(ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(result.key))
                deletes.add(delete)
                if (!isAllFields) {
    ...
    override fun getPartitions(query: Query<K, T>): List<PartitionQuery<K, T>> {
        ...
        for (i in 0 until keys.getFirst().size) {
            ...
            val startRow = if (query.startKey != null)
                ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(query.startKey)
            else
                HConstants.EMPTY_START_ROW
            val stopRow = if (query.endKey != null)
                ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(query.endKey)
            else
            ...
            if ((startRow.size == 0 || keys.getSecond()[i].size == 0 ||
                ...
                val startKey = if (Arrays.equals(HConstants.EMPTY_START_ROW, splitStart))
                    ...
                    ai.platon.plusar.gora.hbase.store.HBaseByteInterface.fromBytes(keyClass, splitStart)
                val endKey = if (Arrays.equals(HConstants.EMPTY_END_ROW, splitStop))
                    null
                else
                    ai.platon.plusar.gora.hbase.store.HBaseByteInterface.fromBytes(keyClass, splitStop)
    ...
    override fun execute(query: Query<K, T>): org.apache.gora.query.Result<K, T>? {
        try {
            ...
            if (query.startKey != null && query.startKey == query.endKey) {
                val get = Get(ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(query.startKey))
    ...
    fun createScanner(query: Query<K, T>): ResultScanner {
        ...
        if (query.startKey != null) {
            scan.startRow = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(query.startKey)
        }
        if (query.endKey != null) {
            ...
            val endKey = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.toBytes(query.endKey)
        ...
        when (fieldSchema.type) {
            ...
            Schema.Type.MAP -> {
                ...
                for ((key, value) in qualMap) {
                    map[Utf8(Bytes.toString(key))] = ai.platon.plusar.gora.hbase.store.HBaseByteInterface.fromBytes(valueSchema, value)
                }
            ...
            Schema.Type.ARRAY -> {
                ...
                for (e in qualMap.entries) {
                    dirtyListWrapper.add(ai.platon.plusar.gora.hbase.store.HBaseByteInterface.fromBytes(valueSchema, e.value))
                }
    ...
    private fun setField(persistent: T, field: Schema.Field, value: ByteArray?) {
        persistent.put(field.pos(), ai.platon.plusar.gora.hbase.store.HBaseByteInterface.fromBytes(field.schema(), value))
    }
