...
public class HtmlParserTest {
    ...
    @Test public void dropsUnterminatedTag() {
        // dom used to parse this to <p>, but whatwg, webkit will drop.
        String h1 = "<p";
    ...
    @Test public void dropsUnterminatedAttribute() {
        // dom used to parse this to <p id="foo">, but whatwg, webkit will drop.
        String h1 = "<p id=\"foo";
        Document doc = Jsoup.parse(h1);
        assertEquals("", doc.text());
    }
    ...
    @Test public void testSelectWithOption() {
        ...
        Document document = parser.parseInput("<select><option>Option 1</option></select>", "http://dom.org");
    ...
    @Test public void createsDocumentStructure() {
        String html = "<meta name=keywords /><link rel=stylesheet /><title>dom</title><p>Hello world</p>";
        Document doc = Jsoup.parse(html);
    ...
    @Test public void doesNotCreateImplicitLists() {
        // old dom used to wrap this in <ul>, but that's not to spec
    ...
    @Test public void discardsNakedTds() {
        // dom used to make this into an implicit table; but browsers make it into a text run
    ...
    @Test public void handlesUnclosedDefinitionLists() {
        // dom used to create a <dl>, but that's not to spec
    ...
    @Test public void testHgroup() {
        // dom used to not allow hroup in h{n}, but that's not in spec, and browsers are OK
    ...
    @Test public void testNoImagesInNoScriptInHead() {
        // dom used to allow, but against spec if parsing with noscript
    ...
    @Test public void noImplicitFormForTextAreas() {
        // old dom parser would create implicit forms for form children like <textarea>, but no more
