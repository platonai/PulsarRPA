...
public class NodeTest {
    ...
    @Test public void handlesAbsPrefix() {
        Document doc = Jsoup.parse("<a href=/foo>Hello</a>", "https://dom.org/");
        Element a = doc.select("a").first();
        assertEquals("/foo", a.attr("href"));
        assertEquals("https://dom.org/foo", a.attr("abs:href"));
        assertTrue(a.hasAttr("abs:href"));
    }

    @Test public void handlesAbsOnImage() {
        Document doc = Jsoup.parse("<p><img src=\"/rez/osi_logo.png\" /></p>", "https://dom.org/");
        Element img = doc.select("img").first();
        assertEquals("https://dom.org/rez/osi_logo.png", img.attr("abs:src"));
    ...
    @Test public void handlesAbsPrefixOnHasAttr() {
        ...
        assertEquals("https://dom.org/", two.absUrl("href"));
    }
    ...
    @Test public void absHandlesRelativeQuery() {
        Document doc = Jsoup.parse("<a href='?foo'>One</a> <a href='bar.html?foo'>Two</a>", "https://dom.org/path/file?bar");

        Element a1 = doc.select("a").first();
        assertEquals("https://dom.org/path/file?foo", a1.absUrl("href"));

        Element a2 = doc.select("a").get(1);
        assertEquals("https://dom.org/path/bar.html?foo", a2.absUrl("href"));
    }
