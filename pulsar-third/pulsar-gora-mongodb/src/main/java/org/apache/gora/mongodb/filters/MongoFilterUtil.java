...
public class MongoFilterUtil<K, T extends PersistentBase> {
    ...
    public static final Logger LOG = LoggerFactory.getLogger(MongoFilterUtil.class);
    ...
    public MongoFilterUtil(final Configuration conf) throws GoraException {
        String[] factoryClassNames = conf.getStrings(MONGO_FILTER_FACTORIES_PARAMETER, MONGO_FILTERS_DEFAULT_FACTORY);
     ...
     */
    public boolean setFilter(final Document query, final Filter<K, T> filter, final MongoStore<K, T> store) {
        ...
        if (factory == null) {
            ...
            Document mongoFilter = factory.createFilter(filter, store);
            if (mongoFilter == null) {
