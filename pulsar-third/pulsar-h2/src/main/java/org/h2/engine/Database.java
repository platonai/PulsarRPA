...
public class Database implements DataHandler {
    ...
    public FileStore openFile(String name, String openMode, boolean mustExist) {
        ...
        try {
            // Read from file system if exists or write a header to file system
            store.init();
        } catch (DbException e) {
            store.closeSilently();
            throw e;
    ...
    private synchronized void open(int traceLevelFile, int traceLevelSystemOut) {
        ...
        // Load all records into memory from disk
        metaTable = mainSchema.createTable(data);
        // Load all index data
        IndexColumn[] pkCols = IndexColumn.wrap(new Column[] { columnId });
        metaIdIndex = metaTable.addIndex(systemSession, "SYS_ID",
                0, pkCols, IndexType.createPrimaryKey(
                false, false), true, null);
        objectIds.set(0);
        starting = true;

        // Execute sql in meta table to create DBObjects: Table, Index, Constraint, UserFunction, etc
    ...
    private synchronized void addMeta(Session session, DbObject obj) {
        ...
        if (id > 0 && !starting && !obj.isTemporary()) {
            ...
            if (isMultiVersion()) {
                ...
                // moment
                session.log(metaTable, UndoLogRecord.INSERT, r);
            }
        }
    }
    ...
    public void verifyMetaLocked(Session session) {
        if (metaTable != null && !metaTable.isLockedExclusivelyBy(session)
    ...
    public boolean lockMeta(Session session) {
        ...
        boolean wasLocked = metaTable.lock(session, true, true);
        return wasLocked;
    }
    ...
    public synchronized void removeMeta(Session session, int id) {
        if (id > 0 && !starting) {
            SearchRow r = metaTable.getTemplateSimpleRow(false);
            ...
            if (cursor.next()) {
                ...
                if (isMultiVersion()) {
                    ...
                    // the moment
                    session.log(metaTable, UndoLogRecord.DELETE, found);
                }
                if (SysProperties.CHECK) {
