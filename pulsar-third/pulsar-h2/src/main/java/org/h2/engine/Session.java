...
public class Session extends SessionWithState {
    ...
    private Map<String, CommandFlow> commandFlows = new HashMap<>();
    // Public command's sequence
    ...
    /**
     * Get a copy of all local temp tables
     * */
     ...
     */
    public void addLocalTempTable(Table table, boolean replaceIfExists) {
        if (localTempTables == null) {
            localTempTables = database.newStringMap();
        }
        if (!replaceIfExists && localTempTables.get(table.getName()) != null) {
    ...
    public Prepared prepare(String sql, boolean rightsChecked, boolean literalsChecked) {
        // System.out.println("==Session::prepare==" + sql);
    ...
    /**
     * TODO: The difference between a Prepared and a Command
     * A command has method executeQuery and executeUpdate
     * A prepared has method query and update
     * A prepared is a light command, and a command is a container of prepared
     */
    public Command prepareCommand(String sql) {
        // System.out.println("==Session::prepareCommand==" + sql);
        // out.println("\n\n\n==Session::prepareCommand==" + sql);
        ++commandSequence;
        CommandFlow commandFlow = getCommandFlow(sql);

        try {
            prepareCommand(commandFlow);
        } finally {
            subQueryIndexCache = null;
        }

        if (queryCache != null && commandFlow.isCacheable()) {
            queryCache.put(sql, commandFlow);
        }

        // Clean up the current command flow
        // TODO: may do it somewhere else
        removeCommandFlow(commandFlow);

        return commandFlow;
    }

    private CommandFlow getCommandFlow(String sql) {
        ...
        if (queryCacheSize > 0) {
            if (queryCache == null) {
                ...
                commandFlow = (CommandFlow) queryCache.get(sql);

                if (commandFlow != null) {
                    trace.debug("==Session::getCommandFlow::queryCache==");
                }

                if (commandFlow != null && commandFlow.canReuse()) {
                    trace.debug("==Session.prepareCommand -> reuse command==");
                    commandFlow.reuse();
                    return commandFlow;
                }
            }
        ...
        commandFlow = commandFlows.get(sql);
        if (commandFlow == null) {
            commandFlow = new CommandFlow(this, sql);
            // TODO: what if execute the sql twice in a single session?
            // Failed to execute TestScript in which a sql executed twice in a single session
            // commandFlows.put(sql, commandFlow);
        }

        return commandFlow;
    }

    private void removeCommandFlow(CommandFlow commandFlow) {
        commandFlows.remove(commandFlow.getSql());
    }
     ...
     *
     * The current command shares select items, tables and result set of upstream command if exist
     *
     * @param commandFlow the command list for the current sql
     */
    private void prepareCommand(CommandFlow commandFlow) {
        if (closed) {
            throw DbException.get(ErrorCode.CONNECTION_BROKEN_1, "session closed");
        }

        // System.out.println("Session.prepareCommand, upstream: >>>" + (upstream == null ? "" : upstream.toString()) + " <<<");
        // prepare the first sql statement
        commandFlow.prepare();

        // TODO: should we do it here? why not do it inside CommandFlow?
        commandFlow.prepareJoinBatch();
    }
