...
public class DataType {
    ...
    private static void add(int valueType, int sqlType, String jdbc, DataType dataType, String[] names, int memory) {
        for (int i = 0; i < names.length; i++) {
            ...
            TYPES_BY_VALUE_TYPE.putIfAbsent(valueType, dt);
            TYPES.add(dt);
        }
    }
    ...
    public static Object convertTo(JdbcConnection conn, Value v, Class<?> paramClass) {
        ...
        switch (v.getType()) {
        case Value.JAVA_OBJECT: {
            // System.out.println("== JAVA_OBJECT convertTo " + DataType.getTypeClassName(v.getType()) + " " + paramClass);
        ...
        throw DbException.getUnsupportedException("converting to class "
                + paramClass.getName() + " from type " + v.getType());
    }
