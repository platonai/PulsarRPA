...
public class PageStore implements CacheWriter {
    ...
    private void readStaticHeader() {
        // go cross database header
        file.seek(FileStore.HEADER_LENGTH);
        // static header length
        final int len = PAGE_SIZE_MIN - FileStore.HEADER_LENGTH;
        Data page = Data.create(database, new byte[len]);
        // read to page data
        file.readFully(page.getBytes(), 0, len);
    ...
    private void readVariableHeader() {
        ...
        for (int i = 1;; i++) {
            ...
            readPage(i, pageData);
            CRC32 crc = new CRC32();
            crc.update(pageData.getBytes(), 4, pageSize - 4);
            int expected = (int) crc.getValue();
            int got = pageData.readInt();
            if (expected == got) {
                writeCountBase = pageData.readLong();
                logKey = pageData.readInt();
                logFirstTrunkPage = pageData.readInt();
                logFirstDataPage = pageData.readInt();
                break;
            }
        }
    }
     ...
     * Read a page.
     *
     * @param pageId the i-th page
     * @param page the page
     */
    void readPage(int pageId, Data page) {
        if (recordPageReads) {
            if (pageId >= MIN_PAGE_COUNT && recordedPagesIndex.get(pageId) == IntIntHashMap.NOT_FOUND) {
                recordedPagesIndex.put(pageId, recordedPagesList.size());
                recordedPagesList.add(pageId);
            }
        }

        if (pageId < 0 || pageId >= pageCount) {
            throw DbException.get(ErrorCode.FILE_CORRUPTED_1, pageId + " of " + pageCount);
        }
