...
public class Function extends Expression implements FunctionCall {
    ...
    public static final int EXPLODE = 1100;
    public static final int POSEXPLODE = 1100;
    public static final int IMPLODE = 1101;
    ...
    static {
        ...
        addFunctionNotDeterministic("SYSTIMESTAMP", CURRENT_TIMESTAMP, VAR_ARGS, Value.TIMESTAMP);
        ...
        // addFunctionWithNull("OUT_PAGE_TABLE", TABLE, VAR_ARGS, Value.RESULT_SET);
        ...
        case CASE: {
            ...
            if (v0 == null) {
                ...
                for (int i = 1, len = args.length - 1; i < len; i += 2) {
                    ...
                    if (!(when == ValueNull.INSTANCE) && when.getBoolean()) {
                        then = args[i + 1];
                        break;
                    }
                }
    ...
    private static long getDiskSpaceUsed(Session session, Value v0) {
        ...
        Table table = Parser.parseTableName(session, sql);
    ...
    private Sequence getSequence(Session session, Value v0, Value v1) {
        ...
        if (v1 == null) {
            String sql = v0.getString();
            Expression expr = Parser.parseExpression(session, sql);
    ...
    protected void checkParameterCount(int len) {
        ...
        if (!ok) {
            throw DbException.get(ErrorCode.INVALID_PARAMETER_COUNT_2, info.name, min + "src/main" + max);
        }
    }
