...
public class ExpressionColumn extends Expression {
    ...
    /**
     * Scent query, equals to the column name by default
     * */
    ...
    public ExpressionColumn(Database database, String schemaName, String tableAlias, String columnName) {
        this(database, schemaName, tableAlias, columnName, null);
    }

    public ExpressionColumn(Database database, String schemaName,
                            String tableAlias, String columnName, String squery) {
        this.database = database;
    ...
    public void mapColumns(ColumnResolver resolver, int level) {
        ...
        // upstream result column reference
        // column index based resolving
        if (columnName != null && '_' == columnName.charAt(0)) {
            String s = columnName.substring(1);
            if (StringUtils.isNumber(s)) {
                int columnId = Integer.parseInt(s) - 1;

                int len = resolver.getColumns().length;
                if (columnId >= 0 && columnId < len) {
                    Column col = resolver.getColumns()[columnId];
                    mapColumn(resolver, col, level);
                    return;
                } else {
                    throw DbException.get(ErrorCode.INVALID_VALUE_2, "upstream result column index out of range");
                }
            }
        }

        // column name based resolving
    ...
    public Expression optimize(Session session) {
        if (columnResolver == null) {
            ...
            System.out.println("ExpressionColumn.optimize\t" + columnName);
...
//        System.out.print(select.getExplicitSelectId() + " ");
//        values.forEach((e, o) -> System.out.print(e.getColumnName() + " "));
//        System.out.println();
    ...
    public void createIndexConditions(Session session, TableFilter filter) {
        ...
        if (filter == tf && column.getType() == Value.BOOLEAN) {
            IndexCondition cond = IndexCondition.get(
                    Comparison.EQUAL, this, ValueExpression.get(ValueBoolean.get(true)));
