...
import org.h2.command.CommandFlow;
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        try {
            ...
            synchronized (session) {
                ...
                assert !(conn.getSession() instanceof Session) || command instanceof CommandFlow;
...
//                    System.out.println("==JdbcStatement.executeQuery==\t" + ((Command)command).getSql());
//                    while (result.next()) {
//                        System.out.println(result.currentRow()[1].getString());
//                    }
                    lazy = result.isLazy();
                } finally {
                    if (!lazy) {
