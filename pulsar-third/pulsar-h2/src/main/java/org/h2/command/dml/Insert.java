...
public class Insert extends Prepared implements ResultTarget {
    ...
    private final ArrayList<Expression[]> expressionsList = New.arrayList();
    private Query query;
    ...
    private int insertRows() {
        ...
        rowNumber = 0;
        int listSize = expressionsList.size();
        if (listSize > 0) {
            int columnLen = columns.length;
            for (int x = 0; x < listSize; x++) {
                session.startStatementWithinTransaction();
                Row newRow = table.getTemplateRow();
                Expression[] expr = expressionsList.get(x);
    ...
    public String getPlanSQL() {
        ...
        if (expressionsList.size() > 0) {
            ...
            for (Expression[] expr : expressionsList) {
                if (row++ > 0) {
                    buff.append(",\n");
                }
                buff.append('(');
    ...
    public void prepare() {
        if (columns == null) {
            if (expressionsList.size() > 0 && expressionsList.get(0).length == 0) {
    ...
    private void handleOnDuplicate(DbException de) {
        ...
        for (int i = 0; i < columns.length; i++) {
            ...
            session.setVariable(key,
              expressionsList.get(getCurrentRowNumber() - 1)[i].getValue(session));
        }
