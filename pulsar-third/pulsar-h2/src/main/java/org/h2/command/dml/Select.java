...
public class Select extends Query {
    ...
    /**
     * The order of select keyword in the sql statement, for example:
     *
     * SELECT a, b FROM t1
     * UNION
     * (
     *   SELECT c, d FROM t2 LEFT JOIN SELECT e, f FROM t3 ON t2.c = t3.e)
     *   UNION VALUES(('a', 1), ('b', 2)
     * )
     *
     * There are three explicit select and a implicit select
     * */
    private int explicitSelectId;
    private final ArrayList<TableFilter> filters = New.arrayList();
    private final ArrayList<TableFilter> topFilters = New.arrayList();
    private ArrayList<Expression> unExpendedExpressions;
    private ArrayList<Expression> expressionColumns;
    private ArrayList<Expression> expressions;
    private Expression[] outExpressions;
    ...
    public Select(int explicitSelectId, Session session) {
        super(session);
        this.explicitSelectId = explicitSelectId;
    }
    ...
    public int getExplicitSelectId() {
        return explicitSelectId;
    }

    public void setExpressions(ArrayList<Expression> expressions) {
        this.unExpendedExpressions = expressions;
        this.expressions = expressions;
    }

    public ArrayList<Expression> getUnExpendedExpressions() {
        return unExpendedExpressions;
    }

    public Expression getUnExpendedExpression(int i) {
        return unExpendedExpressions.get(i);
    }
    ...
    private LazyResult queryGroupSorted(int columnCount, ResultTarget result) {
        LazyResultGroupSorted lazyResult = new LazyResultGroupSorted(outExpressions, columnCount);
        if (result == null) {
            return lazyResult;
        }
        while (lazyResult.next()) {
            // result.addRow(lazyResult.currentRow());
            addRow(lazyResult.currentRow(), result);
        }
        return null;
    }
...
//        return condition == null || condition.getBooleanValue(session);
    ...
    private LazyResult queryFlat(int columnCount, ResultTarget result, long limitRows) {
        ...
        int sampleSize = getSampleSizeValue(session);
        LazyResultQueryFlat lazyResult = new LazyResultQueryFlat(outExpressions, sampleSize, columnCount);
        if (result == null) {
            return lazyResult;
        }
        while (lazyResult.next()) {
    ...
    private void addRow(Value[] row, ResultTarget result) {
        // explode function is support only when there is just a single column
        // TODO: bugy, column metadata does not match the value type
        if (row.length == 1 && row[0].getType() == Value.RESULT_SET) {
            ValueResultSet vrs = (ValueResultSet)row[0];
            if (vrs.isExplode()) {
                ResultSet rs = vrs.getResultSet();
                try {
                    int type = DataType.getValueTypeFromResultSet(rs.getMetaData(), 1);
                    while (rs.next()) {
                        Value v = DataType.convertToValue(session, rs.getObject(1), type);
    ...
    public ResultInterface queryMeta() {
        LocalResult result = new LocalResult(session, outExpressions, visibleColumnCount);
        result.done();
        return result;
    }
        ...
        if (result != null) {
            ...
            if (target != null) {
                while (result.next()) {
                    ...
                    addRow(result.currentRow(), target);
                }
                result.close();
                return null;
            }
    ...
    private LocalResult createLocalResult(LocalResult old) {
        return old != null ? old : new LocalResult(session, outExpressions, visibleColumnCount);
    }
    ...
    public void init() {
        ...
        if (expressions.size() == 1 && expressions.get(0) instanceof JavaFunction) {
            JavaFunction fun = (JavaFunction) expressions.get(0);
            if (fun.getName().endsWith("explode") && fun.getType() == Value.RESULT_SET) {
        ...
        if (group != null) {
            Objects.requireNonNull(expressionSQL);
    ...
    public void beforePrepare() {
        for (TableFilter topFilter : topFilters) {
            topFilter.beforePrepare();
        }
    }

    @Override
    public void prepare() {
        // trace.debug("==Select.prepare==");
        ...
        if (distinct && session.getDatabase().getSettings().optimizeDistinct &&
            ...
            if (expr instanceof ExpressionColumn) {
                ...
                Table table = topTableFilter.getTable();
                Index columnIndex = table.getIndexForColumn(column, false, true);
            ...
            if (index != null && current != null) {
                if (current.getIndexType().isScan() || current == index) {
                    ...
                    if (!topTableFilter.hasInComparisons()) {
                        ...
                        && index.getIndexColumns().length >= current.getIndexColumns().length) {
        ...
        outExpressions = new Expression[expressions.size()];
        expressions.toArray(outExpressions);
    ...
    private double preparePlan(boolean parse) {
        // trace.debug("==Select.preparePlan==");
    ...
    public String getPlanSQL() {
        ...
        for (int i = 0; i < visibleColumnCount; i++) {
            ...
            int sequence = exprList[i].getSelectItemSequence();
            if (sequence > 0) {
                // buff.append(" /* _" + sequence + " */");
            }
        }
        buff.append("\nFROM ");
