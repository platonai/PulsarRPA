...
public class Column {
    ...
    /**
     * Css Query, XPath, Regex, Sigma DOM Query
     * */
    private String squery;
    /**
     * Automatically generated alias, _1, _2, _3, etc
     * */
    ...
    public Value convert(Value v, Mode mode) {
        try {
            ...
            if (e.getErrorCode() == ErrorCode.DATA_CONVERSION_ERROR_1) {
                ...
                throw DbException.get(ErrorCode.DATA_CONVERSION_ERROR_1, e, v.getSQL() + " (" + target + ")");
            }
            throw e;
        }
    }
    ...
    synchronized Value computeValue(Session session, Row row) {
        if (squery != null) {
            if (table instanceof ExternalTableInterface) {
                // table.getDocument().query(name);
                // Or automatically wrap by a user defined function e()?
            }
        }
    ...
    public void prepareExpression(Session session) {
        if (defaultExpression != null) {
            computeTableFilter = new TableFilter(session,
              table, null, false, null, 0, null);
