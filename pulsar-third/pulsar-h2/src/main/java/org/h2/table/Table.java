...
public abstract class Table extends SchemaObjectBase {
    ...
    /**
     * A tool to create columns using simple syntax, can move to a utility class
     * */
    public static ArrayList<Column> createColumns(List<String> names) {
        ArrayList<Column> cols = new ArrayList<>(names.size());
        for (int i = 0; i < names.size(); i++) {
            ...
            if (pos >= 0) {
                ...
                String dataTypeName = name.substring(pos + offset).toUpperCase();
                dataType = DataType.getTypeByName(dataTypeName).type;
            ...
            cols.add(new Column(name, dataType));
        }
