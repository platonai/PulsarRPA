...
import org.h2tools.jaqu.util.ClassUtils;
import org.h2tools.jaqu.util.StatementLogger;
...
class TableDefinition<T> {
     ...
     * @param modelFields the ordered list of model fields
     */
    void addIndex(Table.IndexType type, Object[] modelFields) {
        List<String> columnNames = mapColumnNames(modelFields);
     ...
     * @param columnNames the ordered list of column names
     */
    void addIndex(Table.IndexType type, List<String> columnNames) {
    ...
    void mapFields() {
        ...
        if (clazz.isAnnotationPresent(Table.JQTable.class)) {
            Table.JQTable tableAnnotation = clazz.getAnnotation(Table.JQTable.class);
        ...
        for (Field f : classFields) {
            ...
            String defaultValue = "";
            boolean hasAnnotation = f.isAnnotationPresent(Table.JQColumn.class);
            if (hasAnnotation) {
                Table.JQColumn col = f.getAnnotation(Table.JQColumn.class);
    ...
    void mapObject(Object obj) {
        ...
        if (clazz.isAnnotationPresent(Table.JQSchema.class)) {
            Table.JQSchema schemaAnnotation = clazz.getAnnotation(Table.JQSchema.class);
        ...
        if (clazz.isAnnotationPresent(Table.JQIndex.class)) {
            Table.JQIndex indexAnnotation = clazz.getAnnotation(Table.JQIndex.class);

            // setup the indexes, if properly annotated
            addIndexes(Table.IndexType.STANDARD, indexAnnotation.standard());
            addIndexes(Table.IndexType.UNIQUE, indexAnnotation.unique());
            addIndexes(Table.IndexType.HASH, indexAnnotation.hash());
            addIndexes(Table.IndexType.UNIQUE_HASH, indexAnnotation.uniqueHash());
        }
    }

    void addIndexes(Table.IndexType type, String [] indexes) {
    ...
    List<IndexDefinition> getIndexes(Table.IndexType type) {
