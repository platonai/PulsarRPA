...
import static org.h2tools.jaqu.ValidationRemark.warn;
...
public class TableInspector {
    ...
    void read(DatabaseMetaData metaData) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (info.type.equals(Table.IndexType.UNIQUE)
    ...
    String generateModel(String packageName, boolean annotateSchema,
        ...
        imports.add(Table.JQSchema.class.getCanonicalName());
        imports.add(Table.JQTable.class.getCanonicalName());
        imports.add(Table.JQIndex.class.getCanonicalName());
        imports.add(Table.JQColumn.class.getCanonicalName());
        ...
        if (annotateSchema && !StringUtils.isNullOrEmpty(schema)) {
            model.append('@').append(Table.JQSchema.class.getSimpleName());
            model.append('(');
        ...
        // @JQTable
        model.append('@').append(Table.JQTable.class.getSimpleName());
        model.append('(');
        ...
        generateIndexAnnotations(ap, "standard", Table.IndexType.STANDARD);
        generateIndexAnnotations(ap, "unique", Table.IndexType.UNIQUE);
        generateIndexAnnotations(ap, "hash", Table.IndexType.HASH);
        generateIndexAnnotations(ap, "uniqueHash", Table.IndexType.UNIQUE_HASH);
        if (ap.length() > 0) {
            model.append('@').append(Table.JQIndex.class.getSimpleName());
            model.append('(');
            model.append(ap);
            model.append(')').append(EOL);
        }
    ...
    private List<IndexInspector> getIndexes(Table.IndexType type) {
    ...
    private StatementBuilder generateColumn(Set<String> imports,
        ...
        if (clazz == null) {
            ...
            sb.append('@').append(Table.JQColumn.class.getSimpleName());
    ...
    <T> List<ValidationRemark> validate(TableDefinition<T> def,
        ...
        if (!Modifier.isPublic(def.getModelClass().getModifiers())) {
            remarks.add(ValidationRemark.error(
                    table,
                    "SCHEMA",
        ...
        if (!StringUtils.isNullOrEmpty(schema)) {
            if (StringUtils.isNullOrEmpty(def.schemaName)) {
                remarks.add(ValidationRemark.consider(
                        table,
                        "SCHEMA",
                        MessageFormat.format("@{0}(name={1})",
                                Table.JQSchema.class.getSimpleName(), schema)));
            } else if (!schema.equalsIgnoreCase(def.schemaName)) {
                remarks.add(ValidationRemark.error(
                        table,
                        "SCHEMA",
                        MessageFormat.format("@{0}(name={1}) != {2}",
                                Table.JQSchema.class.getSimpleName(), def.schemaName,
        ...
        for (TableDefinition.FieldDefinition fieldDef : def.getFields()) {
    ...
    private <T> void validate(List<ValidationRemark> remarks,
        ...
        List<TableDefinition.IndexDefinition> defIndexes = def.getIndexes(Table.IndexType.STANDARD);
        List<IndexInspector> dbIndexes = getIndexes(Table.IndexType.STANDARD);
        if (defIndexes.size() > dbIndexes.size()) {
            remarks.add(warn(table, Table.IndexType.STANDARD.name(),
    ...
    private void validate(List<ValidationRemark> remarks,
                          TableDefinition.FieldDefinition fieldDef, boolean throwError) {
        // unknown field
        String field = forceUpperCase ? fieldDef.columnName.toUpperCase()
                : fieldDef.columnName;
        if (!columns.containsKey(field)) {
            // unknown column mapping
            remarks.add(ValidationRemark.error(table, fieldDef, "Does not exist in database!")
        ...
        if (!fieldClass.equals(jdbcClass)) {
            if (Number.class.isAssignableFrom(fieldClass)) {
                remarks.add(ValidationRemark.warn(
                        table,
                        col,
                        MessageFormat
        ...
        if (fieldClass == String.class) {
            if ((fieldDef.maxLength != col.size)
                    && (col.size < Integer.MAX_VALUE)) {
                remarks.add(ValidationRemark.warn(table, col, MessageFormat.format(
                        "{0}.maxLength={1}, ColumnMaxLength={2}",
                        Table.JQColumn.class.getSimpleName(), fieldDef.maxLength,
                        col.size)));
            }
            if (fieldDef.maxLength > 0 && !fieldDef.trimString) {
                remarks.add(ValidationRemark.consider(table, col, MessageFormat.format(
                        "{0}.truncateToMaxLength=true"
                                + " will prevent RuntimeExceptions on"
                                + " INSERT or UPDATE, but will clip data!",
                        Table.JQColumn.class.getSimpleName())));
            }
        }
        ...
        if (fieldDef.isAutoIncrement != col.isAutoIncrement) {
            remarks.add(ValidationRemark.warn(table, col, MessageFormat.format(
                    ...
                    Table.JQColumn.class.getSimpleName(), fieldDef.isAutoIncrement,
                    col.isAutoIncrement)));
        }
        // default value
        if (!col.isAutoIncrement && !col.isPrimaryKey) {
            ...
            if (StringUtils.isNullOrEmpty(fieldDef.defaultValue)
                ...
                remarks.add(ValidationRemark.warn(table, col, MessageFormat.format(
                        ...
                        Table.JQColumn.class.getSimpleName(), col.defaultValue)));
                ...
                remarks.add(ValidationRemark.warn(table, col, MessageFormat.format(
                        ...
                        Table.JQColumn.class.getSimpleName(), fieldDef.defaultValue)));
            } else if (!StringUtils.isNullOrEmpty(fieldDef.defaultValue)
                    && !StringUtils.isNullOrEmpty(col.defaultValue)) {
            ...
            if (!ModelUtils.isValidDefaultValue(fieldDef.field.getType(),
                ...
                remarks.add(ValidationRemark.error(table, col, MessageFormat.format(
    ...
    private static class IndexInspector {
        ...
        public IndexInspector(ResultSet rs) throws SQLException {
            ...
            if (!hash && !unique) {
                type = Table.IndexType.STANDARD;
            } else if (hash && unique) {
                type = Table.IndexType.UNIQUE_HASH;
            } else if (unique) {
                type = Table.IndexType.UNIQUE;
            } else if (hash) {
                type = Table.IndexType.HASH;
            }
