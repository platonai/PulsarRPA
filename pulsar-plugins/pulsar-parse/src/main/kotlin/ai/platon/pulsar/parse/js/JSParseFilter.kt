...
class JSParseFilter(val conf: ImmutableConfig) : AbstractParseFilter(), Parser {
        ...
        if (n is Element) {
            ...
            if (name.equals("script", ignoreCase = true)) {
                ...
                for (i in 0 until len) { // Window: onload,onunload
                    ...
                    var links = ArrayList<HyperlinkPersistable>()
     ...
     */
    private fun getJSLinks(plainText: String, anchor: String, base: String): ArrayList<HyperlinkPersistable> {
        val hypeLinks = ArrayList<HyperlinkPersistable>()
        ...
        try {
            ...
            while (matcher.contains(input, pattern)) {
                ...
                hypeLinks.add(HyperlinkPersistable(url, anchor))
            }
        } catch (ex: Exception) {
