...
import ai.platon.pulsar.crawl.fetch.FetchTaskStream
 ...
 */
class AsyncBrowserEmulatedFetcher(
    ...
    private val log = LoggerFactory.getLogger(AsyncBrowserEmulatedFetcher::class.java)!!
     ...
     * Fetch page content
     * */
    suspend fun fetchContent(page: WebPage): Response {
        if (closed.get()) {
    ...
    suspend fun parallelFetchAllPages(pages: Sequence<WebPage>, volatileConfig: VolatileConfig): List<Response> {
        FetchTaskStream(pages, volatileConfig, privacyContextManager.activeContext).use { stream ->
...
//                val future = executor.submit(Callable { fetchContent(page) })
//                future.addListener {
//                }
            }
