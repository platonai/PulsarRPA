...
class TestUrlNormalizers {
    @Autowired
    private val urlNormalizers: ChainedUrlNormalizer? = null
    @Test
    fun testURLNormalizers() {
        Assert.assertEquals(3, urlNormalizers!!.getURLNormalizers(ChainedUrlNormalizer.SCOPE_DEFAULT).size.toLong())
        var url = "http://www.example.com/"
        val normalizedUrl = urlNormalizers.normalize(url, ChainedUrlNormalizer.SCOPE_DEFAULT)
        Assert.assertEquals(url, normalizedUrl)
        url = "http://www.example.org//path/to//somewhere.html"
        val normalizedSlashes = urlNormalizers.normalize(url, ChainedUrlNormalizer.SCOPE_DEFAULT)
        Assert.assertEquals("http://www.example.org/path/to/somewhere.html", normalizedSlashes)

        // check the order
        val impls = urlNormalizers.getURLNormalizers(ChainedUrlNormalizer.SCOPE_DEFAULT)
