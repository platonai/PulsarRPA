...
class InternalProxyServer(
    ...
    private val ident = DateTimeUtil.now("yyyyMMdd")
    private val path = PulsarPaths.get("proxy", "logs", "proxy-$ident.log")
    ...
    private var readyTimeout = Duration.ofSeconds(120)
    ...
    val enabled = PulsarEnv.useProxy && conf.getBoolean(PROXY_ENABLE_INTERNAL_SERVER, true)
    ...
    fun waitUntilRunning(): Boolean {
        ...
        lock.withLock {
            ...
            if (!b) {
                // timeout
                log.warn("Timeout to wait for proxy server")
            }
        }
