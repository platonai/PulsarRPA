...
open class SeleniumEngine(
    ...
    internal fun fetchTask(task: FetchTask): FetchResult {
    ...
    open fun handleFetchComplete(task: FetchTask, driver: ManagedWebDriver, result: RetrieveContentResult) {
...
//            is HtmlUnitDriver -> page.lastBrowser = BrowserType.HTMLUNIT
    ...
    private fun fetchTaskWithRetry(task: FetchTask): Response {
        ...
        while (i++ < maxRetry && result.response == null && !isClosed && !Thread.currentThread().isInterrupted) {
            ...
            try {
                result = fetchOnce(task, driver)
            } finally {
                handleFetchComplete(task, driver, result)
            }

            val r = result.response
            if (i > 1 && r != null && r.status.isSuccess && r.length() > 100_1000) {
                log.info("Retried {} times and retrieved a good page with {} | {}",
                        i, StringUtil.readableByteCount(r.length()), task.url)
            }
        }
    ...
    private fun fetchOnce(task: FetchTask, driver: ManagedWebDriver): RetrieveContentResult {
        ...
        try {
            if (isISPEnabled) {
                response = internalProxyServer.run { visit(driver, task) }
                driver.proxyEntry = internalProxyServer.proxyEntry
            } else {
                response = visit(driver, task)
            }
    ...
    private fun visit(driver: ManagedWebDriver, task: FetchTask): Response {
        ...
        try {
            val result = visitAndCompute(task, driver.driver, driverConfig)
    ...
    private fun visitAndCompute(task: FetchTask, driver: WebDriver, driverConfig: DriverConfig): VisitResult {
