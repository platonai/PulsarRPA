...
Pulsar reports the status for every page loading in the logs, so it's easy to know what happened in the system, find out answers such as is the system running healthy, how many pages are successfully fetched, how many pages are retrying, how many proxy ips are used, and so on.
