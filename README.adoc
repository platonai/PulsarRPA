...
Extracting web data at scale is extremely hard. #Websites change frequently and are becoming more complex, meaning web data collected is often inaccurate or incomplete#, Pulsar has developed a range of cutting-edge technologies to solve this problem.
...
The code above will scrape fields specified by css selectors #title and #acrCustomerReviewText from a set of product pages. The example code can be found here: link:pulsar-app/pulsar-examples/src/main/kotlin/ai/platon/pulsar/examples/sites/topEc/english/amazon/AmazonCrawler.kt[kotlin].
...
The example code can be found here: link:pulsar-app/pulsar-examples/src/main/kotlin/ai/platon/pulsar/examples/ContinuousCrawler.kt[kotlin], link:pulsar-app/pulsar-examples/src/main/java/ai/platon/pulsar/examples/ContinuousCrawler.java[java].
...
* Auto Extract: learn the data schema automatically and extract every field from webpages, powered by cutting-edge AI solutions
* RPA: stands for robotic process automation which is the only way to scrape modern webpages
* Network As A Database: access the Web just like a local database
* X-SQL: query the Web using SQL directly
* Pulsar Session: a Pulsar session provides a set of simple, powerful and flexible APIs to do web scraping tasks
* Web Driver: the web driver defines a concise interface to visit and interact with webpages, all behaviors are optimized to mimic real people as closely as possible
...
* Event Handler: capture and process events throughout the lifecycle of webpages
...
// load a page, fetch it from the Web if it expires
...
// load, parse and scrape named fields from a set of out pages
...
Pulsar provides a convenient way to mimic real people during the lifecycle of a webpage, using a web driver to interact with the webpage: scrolling, typing, screen capturing, mouse dragging, clicking and more, all actions and behaviors are optimized to mimic real people as closely as possible.

Here is a typical RPA code snippet, which is required to collect data from top e-comm sites.
...
== Usage: experience Pulsar with the executable jar

We have released a standalone, executable jar, based on Pulsar. Download link:https://github.com/platonai/exotic#download[Exotic] and explore its capabilities with a single command line:

    java -jar exotic-standalone.jar

== Log & Metrics

Pulsar has carefully designed the log and metrics subsystem, to record every event happen in the system.

Pulsar reports the status for every page load execution in the logs, so it's easy to know what happened in the system, find out answers such as is the system running healthy, how many pages were successfully fetched, how many pages were retried, how many proxy ips were used, etc.

A typical page loading report is as the following, and you can check link:docs/log-format.adoc[log-format] to learn how to read the logs to learn the status of the whole system at a glance.
