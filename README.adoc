...
The most important load options are:
    ...
    -refresh     // Force (re)fetch the page, just like the refresh button on a real browser
    -parse       // Triger the parse phrase
    -isResource  // Fetch the url as a resource without browser rendering

All load options are parsed to a link:pulsar-skeleton/src/main/kotlin/ai/platon/pulsar/common/options/LoadOptions.kt[LoadOptions], check the code for all possible options.
...
Pulsar is tested on Ubuntu 18.04, Ubuntu 20.04, Windows 11, WSL, any other OS matches the requirements might also be OK.
