...
*PulsarR supports the Network-As-A-Database paradigm.* PulsarR treats the external network as a database. If the required data is not in the local storage, or the existing version does not meet the analysis needs, the system will collect the latest version of the data from the Internet. We can also use simple SQL to directly convert the Web into tables and charts. Furthermore, we can use SQL to query the Web directly.

**PulsarR supports browser rendering as the primary method to collect Web data.** By using browser rendering as the primary method to collect Web data, we achieve an optimal balance between data point scale, data quality, labor cost and hardware cost, and achieve the lowest TCO (total cost of ownership). Through optimization, such as blocking irrelevant resource files, the performance of browser rendering can even be comparable to the traditional single resource collection methods. Furthermore, for some resources, we can call PulsarR's single resource scraping methods for high-speed collection after opening the browser and obtaining the context such as cookies and headers.

*PulsarR supports RPA based Web scraping.* PulsarR includes an RPA subsystem for Web interaction: scrolling, typing, screen capture, mouse drag and drop, clicking, etc. This subsystem is similar to the well-known selenium, playwright, puppeteer, but all behaviors are optimized, such as more realistic simulation, better execution performance, better parallelism, better fault tolerance, and so on.

*PulsarR supports degraded single resource collection.* PulsarR's default data collection method is to harvest the complete Web data through browser rendering, but if the data you need can be retrieved through a single API, for example through an ajax interface, you can also call PulsarR's resource scraping method for high-speed collection.
...
* Web spider: browser rendering, ajax data crawling, data collection through plain protocols, etc
...
== Usage I: use Pulsar as a library
...
[source,kotlin]
----
implementation("ai.platon.pulsar:pulsar-all:1.10.3")
----
...
== Usage II: run Pulsar as a REST service
...
== Usage III: experience Pulsar with the executable jar
...
* A concise and powerful PulsarR client, which can query the Web and collect data by writing SQL just like the traditional database client
