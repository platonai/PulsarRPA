...
select
    ...
    str_left(dom_first_text(dom, 'a span[data-hook=avp-badge], a span:containsOwn(Verified)'), 32) as `is_verified`,
    str_left(dom_first_text(dom, 'div.a-profile-content span.a-profile-descriptor, div.a-profile-content span:containsOwn(Top Contributor)'), 32) as `is_top_contributor`,
    str_left(dom_first_text(dom, 'div.genome-widget-row a span:containsOwn(VINE VOICE)'), 32) as `is_vine_voice`,
