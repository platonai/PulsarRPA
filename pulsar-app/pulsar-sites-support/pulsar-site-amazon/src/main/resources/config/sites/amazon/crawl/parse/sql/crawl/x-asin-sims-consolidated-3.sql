...
select dom_base_uri(dom)                                                                          as url,
       ...
       dom_first_own_text(dom_owner_body(dom),
                          '#sims-consolidated-3_feature_div h2.a-carousel-heading, ' ||
                          'div[cel_widget_id~=sims-consolidated-3] h2.a-carousel-heading'
         )                                                                                        as carousel_title,
       dom_first_text(dom_owner_body(dom),
                      '#sims-consolidated-3_feature_div h2.a-carousel-heading div.sp_desktop_sponsored_label, ' ||
                      'div[cel_widget_id~=sims-consolidated-3] h2.a-carousel-heading div.sp_desktop_sponsored_label'
         )                                                                                        as is_sponsored,
       ...
       str_substring_after(dom_first_attr(dom, 'div > div > a i.a-icon-star', 'class'),
       ...
       time_first_mysql_date_time(
           dom_attr(dom_select_first(dom_owner_body(dom), '#PulsarMetaInformation'), 'taskTime')) as `task_time`,
       'sims-3'                                                                                   as `ad_type`
from load_and_select(@url,
                     '#sims-consolidated-3_feature_div ol.a-carousel li, ' ||
                     'div[cel_widget_id~=sims-consolidated-3] ol.a-carousel li'
  );
