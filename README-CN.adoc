...
我们发布了一些最大型电商网站的全站数据采集的完整解决方案，这些解决方案满足最高标准的性能、质量和成本要求，他们将永久免费，譬如：
...
Pulsar 支持 RPA 采集。Pulsar 包含一个 RPA 子系统，使用 Web 驱动程序与网页交互：滚动、打字、屏幕捕获、鼠标拖放、点击等。该子系统和大家所熟知的 selenium/playwright/puppeteer 是类似的，但对所有 RPA 行为进行了优化，譬如更好地模拟真实操作，更好的执行性能，更好的容错处理，等等。
...
// 加载一个页面，如果该页面为首次加载，或者该页面已过期，则从互联网上下载该页面
...
请注意，我们的大多数抓取方法都接受一个称为加载参数或加载选项的参数，以控制如何加载/获取网页。
    ...
    -refresh     // 强制重新采集网页
    -parse       // 激活数据解析流程
    -resource    // 将采集任务视作网络资源进行采集，从而不必经过浏览器渲染

点击 link:docs/concepts.adoc#_load_options[Load Options] 查看所有加载选项。
...
Pulsar 提供了一种在网页生命周期中模仿真人的便捷方式，使用 Web 驱动程序与网页交互：滚动、打字、屏幕捕获、鼠标拖放、点击等。这个大家所熟知的 selenium/playwright/puppeteer 是一致的，不同的是，Pulsar 的所有行为针对大规模数据采集优化。
