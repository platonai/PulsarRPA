...
PulsarR 支持网络即数据库范式。我们可以像对待内部数据库一样对待外部网络，如果我们需要的网页不在本地存储中，则系统会从互联网上采集该网页。我们也可以使用简单的 SQL 将 Web 直接转换为表格和图表，更进一步，我们可以直接使用 SQL 查询 Web。
...
PulsarR 支持退化的单一资源采集。PulsarR 的默认采集方式是通过浏览器渲染来采集完整的网页数据，如果您发现您需要的数据可以通过单一链接获取，譬如某个 ajax 接口，也可以调用 PulsarR 的资源采集方法进行高速采集。
