...
Pulsar 支持 RPA 采集。Pulsar 包含一个 RPA 子系统，使用 Web 驱动程序与网页交互：滚动、打字、屏幕捕获、鼠标拖放、点击等。该子系统和大家所熟知的 selenium/playwright/puppeteer 是类似的，但对所有 RPA 行为进行了优化，譬如更好地模拟真实操作，更好的执行性能，更好的容错处理。
