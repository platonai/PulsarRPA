...
PulsarR 是大规模抓取 Web 数据的终极开源方案，支持几乎所有规模和性质的网络数据采集任务。
...
* https://link.zhihu.com/?target=https%3A//github.com/platonai/exotic-amazon[Exotic Amazon]
* https://link.zhihu.com/?target=https%3A//github.com/platonai/exotic/tree/main/exotic-app/exotic-OCR-examples/src/main/kotlin/ai/platon/exotic/examples/sites/walmart[Exotic Walmart]
* https://link.zhihu.com/?target=https%3A//github.com/platonai/exotic/tree/main/exotic-app/exotic-OCR-examples/src/main/kotlin/ai/platon/exotic/examples/sites/food/dianping[Exotic Dianping]

PulsarR 支持高质量的大规模数据采集和处理。PulsarR 开发了一系列基础设施和前沿技术，来保证即使是大规模数据采集场景，也能达到最高标准的性能、质量和总体拥有成本。

PulsarR 支持 **网络即数据库** 范式。PulsarR 像对待内部数据库一样对待外部网络，如果需要的数据不在本地存储中，或者现存版本不满足分析需要，则系统会从互联网上采集该数据的最新版本。我们也可以使用简单的 SQL 将 Web 直接转换为表格和图表，更进一步，我们直接使用 SQL 可以查询 Web。
...
PulsarR 支持退化的单一资源采集。PulsarR 的默认采集方式是通过浏览器渲染来采集完整的网页数据，如果您需要的数据可以通过单一链接获取，譬如可以通过某个 ajax 接口返回，也可以调用 PulsarR 的资源采集方法进行高速采集。

PulsarR 计划支持最前沿的信息提取技术。我们计划发布一个先进的人工智能，以显著的精度自动提取所有网页（譬如商品详情页）中的每一个字段，目前我们提供了一个 https://link.zhihu.com/?target=https%3A//github.com/platonai/exotic%23run-auto-extract[试用版本]。
...
点击 link:docs/concepts.adoc#_the_core_concepts_of_pulsar[PulsarR concepts] 查看详情。

== 使用方法：将 PulsarR 用作软件库
...
* 一组顶尖站点的数据采集示例
* 基于自监督机器学习的网页数据提取示例，我们的 AI 识别商品详情页的几乎每一个字段，95% 以上字段精确度达到 99% 以上
* 一个简单但同比仍然强大的 PulsarR 客户端，可以像传统数据库客户端一样，通过编写SQL来采集网络数据
