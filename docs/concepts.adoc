...
The Internet, especially, the WWW, is the largest database in the world, but extracting data from the Web has never been easy.
...
We provided a demo project to show how to use our world leading machine learning algorithm to extract almost every field in webpages automatically: https://github.com/platonai/exotic[Exotic].
...
=== Pulsar Session
PulsarSession defines a concise interface for fetching web pages from local storage or the Internet, as well as methods for parsing, extracting, saving, indexing, and exporting web pages.
...
The first thing to understand is how to load a page. The load methods first check the local storage and returns the local version if the required page exists and matches the requirements, otherwise it will fetch it from the Internet.

The loading parameters or loading options can be used to specify when the system will fetch a webpage from the Internet:
...
Since the webpages are becoming more and more complex, fetching webpages through real browsers is the primary way nowadays.
