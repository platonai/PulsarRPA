...
NormUrl stand for `normalized url`, it means the url is the final form to pass to the fetch component, and is usually passed to chrome.

If not specified, an url in string format means a `configured url`, for example:
[source,kotlin]
----
...
A UrlAware provides much more complex controls to the scraping task. UrlAware is the interface for all Hyperlinks, see <<Hyperlink,Hyperlink>> section for details.
...
A ParsableHyperlink is a convenient abstraction to do fetch-and-parse tasks in massive crawling jobs:
...
A CompletableHyperlink helps us to do java style asynchronous computation.
...
fun executeQuery(request: ScrapeRequest): ScrapeResponse {
    ...
    // wait for the scraping complete or timeout
