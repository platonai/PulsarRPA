...
bin/crawl.sh conf/alternatives/information/cluster @conf/alternatives/information/seeds/test_10.txt information_pulsar_`cat VERSION`_`date +%m`01 information_tmp 3 > logs/crawl-information-`date +%Y%m`.out 2>&1 &
# crawl under distribution mode, a bigger seeds collection
bin/crawl.sh conf/alternatives/information/cluster @conf/alternatives/information/seeds/all.txt information_pulsar_`cat VERSION`_`date +%m`01 information_tmp 1000 > logs/crawl-information-`date +%Y%m`.out 2>&1 &
