...
$PREFIX =$VERSION.Split('.')[0,2] -join '.'
$MINOR_VERSION =$VERSION.Split('.')[2]
$MINOR_VERSION = [int]$MINOR_VERSION + 1

$NEXT_VERSION = "$PREFIX.$MINOR_VERSION"$NEXT_SNAPSHOT_VERSION = "$NEXT_VERSION-SNAPSHOT"
...
# Update $APP_HOME/pom.xml$pomXmlPath = "$APP_HOME\pom.xml"
((Get-Content $pomXmlPath) -replace "<tag>v$VERSION<\/tag>", "<tag>v$NEXT_VERSION<\/tag>") | Set-Content $pomXmlPath

# Update pom.xml files
Get-ChildItem "$APP_HOME" -Filter 'pom.xml' -Recurse | ForEach-Object {
