...
class TestString {
    ...
    fun testRegex() {
        ...
        val matcher: Matcher = PATTERN_RECT.matcher(text)
        if (matcher.find()) {
    ...
    fun testReplaceInURL() {
        val url = "https://github.com/platonai/browser4/releases/download/v3.0.14/Browser4.jar"
        val expectedURL = "https://github.com/platonai/browser4/releases/download/v3.0.15/Browser4.jar"
    ...
    fun testStripNonChar() {
        ...
        assertEquals(
            "天王表 正品热卖  个性表盘",
            Strings.removeNonCJKChar("天王表 正品热卖  个性表盘  ", Strings.DEFAULT_KEEP_CHARS)
        )
        assertEquals(
            "天王表 正品热卖 主要职责OK",
            Strings.removeNonCJKChar("天王表 正品热卖 \uE004主要职责：  OK", Strings.DEFAULT_KEEP_CHARS)
        )
    }
