# PulsarAgen#extract / observe 测试指南
...
[PulsarAgentImpl.kt](../agent/PulsarAgentImpl.kt) 主要实现三个方法：`act`, `extract` 与 `observe`。

你需要对 `extract` 与 `observe` 两个方法进行详细测试。

- 代码位置：`pulsar-core/pulsar-skeleton/src/main/kotlin/ai/platon/pulsar/skeleton/ai/agent/PulsarAgentImpl.kt`
- 相关类型：`PulsarAgent`, `PulsarAgentImpl`, `InferenceEngine`, `DomService`

## 测试设计
- **目标**：
    - 在统一的端到端测试框架内测试 `PulsarAgent.extract()`, `PulsarAgent.observe()`。
...
- **Mock 页面选择**：
    - 遵循 `pulsar-tests/src/test/kotlin/ai/platon/pulsar/browser/driver/chrome/dom/ChromeCdpDomServiceE2ETest.kt` 的模式
    - 所有端到端用例均通过 Mock Server（`http://127.0.0.1:18080`）访问 `static/generated` 目录下的交互页面：
